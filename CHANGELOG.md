...
**Native subagents in the Codex delegation surface, comment preferences, interactive git preferences.** Ships alongside claude-agent-kit 12.0.1.

- **Codex delegation surface corrected.** The rule said Codex has no native write-capable subagent and routed all write-capable delegation to dispatch. Codex 0.154.0 ships `spawn_agent` with the `multi_agent` feature enabled by default, and a spawned child has the parent's tools. The insert now says every native spawn is write-capable and goes through GATE-DELEGATE, whatever its `agent_type`; Codex has no native read-only delegate, so read-only second opinions stay with `aside`; a child can spawn its own subagents, so its prompt says whether it may; and dispatch remains the surface for handing a self-contained step to an external backend. The ban on simulating delegation with background shells or nested `codex exec` stays.
- **New: comment preferences.** A user-owned `codex-agent-kit--comment-prefs.md` holds three values: whether new files get a header (`repository` or `structured`), the comment language, and doc comment coverage. Every value defaults to `repository`, so the repository's own convention decides and the file settles only what a repository leaves open. The precedence is the current-turn instruction, then a line under the file's "Repository overrides", then the repository's convention, then the file's value.
...
- **Installers** fetch the new `codex-agent-kit--comment-prefs.md.tmpl`; uninstall keeps the file (user-owned signature).
...
**Subtraction release: blacklist-style rules, task-tracker rule removed, git workflow as user preferences.** The shared rule text was rewritten and cut (65.0 KB to 51.9 KB), and the git conventions became a user-owned prefs file. Ships alongside claude-agent-kit 12.0.0.
...
- **Surface file trimmed.** The `update_plan` tracker bullet, the `rg` / `multi_tool_use.parallel` / `tool_search` bullets, the MCP tool name lists, and the "workslate is Claude-only" line were removed. Kept: the loading model, the goal rules, `apply_patch` and the patch-size note, the clarification rule, and the `no_project_root` containment note.
...
- **rules**: `codex-agent-kit--dispatch.md`'s "Ending a turn" HARD RULE is rewritten — `dispatch_status` is a non-blocking snapshot, so don't tight-poll: do other work and re-check, use the harness's own wait/scheduling, or tell the user. `get_info` / tool descriptions / decision-tree / codex-surface tool list get a clean reference removal, no added guidance.
