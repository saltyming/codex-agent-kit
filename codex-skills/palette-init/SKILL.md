...
Decompose what you learned into backlog items. Use the `_palette/backlog.rst` schema and the RST house style from the always-loaded palette rule (`codex-agent-kit--palette.md`) — do not restate the schema here, follow it. Each discussed feature, requirement, or constraint becomes an item with `:Status: backlog`. Capture genuine durable product principles (product feel, interaction expectations, simplicity / performance / accessibility posture) in the `Core Product Principles` section — 3–7 max, never invented to fill space.
