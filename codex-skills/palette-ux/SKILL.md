...
This is an **optional, pull-only** palette helper. It is not part of the default loop (backlog → slice → stories → hand off → review), and must never run unprompted as part of slicing or story generation. Produce `_palette/ux-flow.rst` only when the user asks, or when the current phase's screens and navigation need defining before stories can be written. Follow the RST house style (robust subset — no tables) in the always-loaded palette rule (`codex-agent-kit--palette.md`).
