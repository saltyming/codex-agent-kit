...
- **Tier A — everything under `_palette/`** (backlog, phase briefs, stories, index, optional artifacts). **Advisory only**: an intent ledger that *proposes*; it becomes executable only after promotion into a current story/plan, user approval, and `update_plan plus goal tracking` tracking.
- **Tier B — the execution machinery.** The user's approval at the "Get approval" gate *authorizes*; `update_plan plus goal tracking` entries *track*. Binding.
...
- **`update_plan plus goal tracking` projection.** Tracker entries for a story are the tactical projection of *already-approved* acceptance criteria — a translation, never a second backlog or new scope.
