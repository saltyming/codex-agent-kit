...
A delegate that hits a forced spec/plan deviation stops and reports to the leader, who then asks the user — a child never shrinks scope, reinterprets a budget, or substitutes a cleaner design on its own (see `codex-agent-kit--task-execution.md` > Forced Spec/Plan Deviation).

**palette note.** When delegating a palette story, hand the delegate the *approved, tracked scope* (as `update_plan plus goal tracking` entries mapped from the story's acceptance criteria), never the raw `story-*.rst`. A story's acceptance criteria pass the approval gate first, then become the delegate's brief — a child receives Tier-B scope, not a Tier-A artifact. See `codex-agent-kit--palette.md`.
