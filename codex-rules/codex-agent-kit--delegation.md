...
When work fans out to delegates: gate, select the mechanism, integrate, verify. This file holds GATE-DELEGATE and the procedures behind INV-GATE-1, INV-GATE-2, and INV-GATE-3 (defined in `AGENTS.md`).
...
Two MCP-backed surfaces sit beside the harness-native ones. `aside` (`codex-agent-kit--aside.md`) is consultation: a read-only second opinion from another model family, and you stay in charge. `dispatch` (`codex-agent-kit--dispatch.md`) is delegation: it hands a write-capable execution step to an external coding agent and tracks it asynchronously, under its own gate (GATE-DISPATCH) and server-enforced guards.
...
A delegate that is forced off its approved spec or plan stops and reports to you, and you ask the user (GATE-DEVIATION in `codex-agent-kit--task-execution.md`). When you delegate a palette story, give the delegate the approved scope, not the raw Tier-A artifact (`codex-agent-kit--palette.md`).
