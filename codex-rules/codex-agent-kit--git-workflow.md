...
**palette note.** `_palette/` (the palette backlog and stories) is the developer's personal planning record and is **not** auto-committed by the agent. `palette-init` offers a `_palette/.gitignore`; a developer who wants to track or share it commits it themselves. See `codex-agent-kit--palette.md`.
...
Destructive git operations (`git reset --hard`, `git checkout -- <file>`, `git restore <file>`, `git clean -f*`, `git stash drop`, `git branch -D`, `git revert`, `git push --force*`, `git rebase` rewriting history) require the same approval gate as session edits: surface the command, the blast radius, the files / commits / branches it would touch, and wait for explicit per-command authorization. They are NEVER a substitute for editing files back when the user asks for a session-edit "undo" — see `codex-agent-kit--task-execution.md` → *Undo / Revert Handling*.
