...
`_palette/` is never auto-committed by the agent (`codex-agent-kit--palette.md` § Gate bindings).
...
Destructive git routes through GATE-GIT (`codex-agent-kit--task-execution.md` → *Undo / Revert Handling* subsection C): explicitly named command only, blast-radius pre-flight, per-command authorization. Never a substitute for editing files back when the user asks for a session-edit "undo" (INV-STATE-2).
