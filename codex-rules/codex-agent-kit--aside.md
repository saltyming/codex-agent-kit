...
**Relation to `dispatch`.** `aside` *asks* (read-only); `dispatch` *does* (write-capable, async). The full taxonomy is stated once in `codex-agent-kit--delegation.md`.
...
1. **Check `$CODEX_HOME/rules/codex-agent-kit--aside-prefs.md` before any aside call** — preferred backend, default model / reasoning effort / fallback chain, auto-call policy. Prefs are the fallback; a user's current-turn instruction always outranks them.
