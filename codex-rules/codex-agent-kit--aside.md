...
**Relation to `dispatch`.** `aside` *asks* (read-only); `dispatch` *does* (write-capable, async). The full aside↔dispatch↔native-delegate taxonomy is stated once in `codex-agent-kit--delegation.md` — don't conflate the surfaces.
...
When the user's `codex-agent-kit--aside-prefs.md` sets `policy: proactive`, you **SHOULD** call the preferred backend on the triggers below. "SHOULD", not "may" — these are active instructions, not permissions. The call is required whether or not a native advisor also exists in this environment (unless the user explicitly scoped the request to one surface — Decision rule 6).
