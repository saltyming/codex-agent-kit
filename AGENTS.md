...
> Operating rules for Codex agents. General tool usage, security,
...
- `codex-agent-kit--task-execution.md` — inner-loop protocol (Understand → Plan → Execute + tactical tracking + scope integrity / undo-revert / forced-deviation gate).
- `codex-agent-kit--palette.md` — product-intent outer loop (optional; activates only when a project has opted in via `_palette/`).
- `codex-agent-kit--delegation.md` — read-only and write-capable delegation gate.
- `codex-agent-kit--aside.md` — read-only cross-family consultation policy for the shared `aside` MCP server.
- `codex-agent-kit--dispatch.md` — write-capable external execution delegation policy for the shared `dispatch` MCP server.
- `codex-agent-kit--git-workflow.md` — commit / PR conventions and destructive-git gate.
...
Full posture (gate-vs-selection split, dependency-structure selection, prompt rules, anti-patterns) is canonical in `codex-agent-kit--delegation.md` — this paragraph is a summary, not a restatement.
...
│     the "Get approval" node below is the palette hand-off (Tier A→B). See codex-agent-kit--palette.md
│
├─ Code change requested?
│  ├─ Multi-step (2+ files / 2+ deliverables)?
│  │  └─ populate update_plan plus goal tracking FIRST
