...
- `codex-agent-kit--task-execution.md` — **the execution loop** (Understand → Plan → Execute), tactical tracking, and the scope/state gates: GATE-SCOPE-CONFIRM, GATE-DEVIATION, GATE-GIT (undo/revert handling).
- `codex-agent-kit--delegation.md` — **the delegation loop**: GATE-DELEGATE, mechanism selection, delegate prompt rules.
- `codex-agent-kit--palette.md` — **the product-intent outer loop** (optional; activates only when a project has opted in via `_palette/`), and the single home of all gate↔palette bindings.
- `codex-agent-kit--aside.md` — the consultation loop for the shared `aside` MCP server (read-only cross-family second opinions).
- `codex-agent-kit--dispatch.md` — the external-execution loop for the shared `dispatch` MCP server (async, write-capable): GATE-DISPATCH.
- `codex-agent-kit--git-workflow.md` — commit / PR conventions; destructive git routes through GATE-GIT.
- `codex-agent-kit--framework-conventions.md` — language-level patterns (React / Next.js, Rust, Python).
...
**INV-SCOPE-2 — Scope is user-owned.** You do not unilaterally decide scope on the user's behalf — not to expand it, not to shrink it, not to substitute a "better approach." When a plan defers scope to post-inspection review ("actual scope decided after reading the code", "코드 확인 후 정한다"), inspection is a user-facing checkpoint: report → propose → wait (procedure: GATE-SCOPE-CONFIRM in `codex-agent-kit--task-execution.md`).

**INV-SCOPE-3 — Deviation only through the gate.** Exactly three situations justify deviating from an approved spec/plan — genuine impossibility, reordering to prevent an ordering-induced regression, or a design that departs from the approved plan — and each one requires stopping, preserving work, proposing the deviation, and waiting for explicit approval (procedure: GATE-DEVIATION in `codex-agent-kit--task-execution.md`). Nothing else justifies deviation at all.

### State

**INV-STATE-1 — No model-initiated rollback.** If you judge mid-work that the direction is wrong or the scope unmanageable, you MUST NOT erase, blank, or hide the incomplete state by any mechanism (destructive git, apply_patch for manual file edits used to overwrite your own work, file deletion, or any other tool). Stop, preserve everything, report, wait. Observable test: if the net effect of an action removes work you created this session *without replacing it with the approved deliverable*, it is rollback — whatever you label it.

**INV-STATE-2 — Undo means file edits.** A user's "revert" / "undo" / "되돌려" defaults to reversing this session's edits via apply_patch for manual file edits — the inverse edit — never via git, which touches repo state including the user's out-of-session work. Destructive git runs only when the user explicitly names the git command, through GATE-GIT (`codex-agent-kit--task-execution.md`).
...
1. **Execution loop** (`codex-agent-kit--task-execution.md`) — Understand → Plan → Execute, per task. Always on.
2. **Delegation loop** (`codex-agent-kit--delegation.md`) — when work fans out to delegates: gate → select mechanism → integrate → verify. `aside` (consult) and `dispatch` (execute externally) are its MCP-backed surfaces.
3. **palette outer loop** (`codex-agent-kit--palette.md`) — backlog → slice → hand off → review, across sessions. Opt-in per project (`_palette/` present); wraps the execution loop and feeds it one approved story at a time.
...
3. **Execute** — implement ALL changes completely, no placeholders (INV-SCOPE-1). Use apply_patch for manual file edits.
...
Read-only delegates: free and proactive. Write-capable delegates: GATE-DELEGATE (INV-GATE-1), then select the mechanism by dependency structure — independent subtasks fan out; coordinated streams need a coordinated mechanism; large mechanical sweeps need a fan-out helper; external execution goes to `dispatch` under its own policy. One writer per file (INV-GATE-2); children inherit everything (INV-GATE-3). Full loop in `codex-agent-kit--delegation.md` — this is a summary, not a restatement.
...
│     "Get approval" node below authorizes (Tier B). See codex-agent-kit--palette.md
│
├─ Code change requested?
│  ├─ Multi-step (2+ files / 2+ deliverables)? → populate update_plan plus goal tracking FIRST
...
│  └─ Implement with apply_patch for manual file edits — entire scope (INV-SCOPE-1)
│
